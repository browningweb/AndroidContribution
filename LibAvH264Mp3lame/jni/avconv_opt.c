...
static int open_output_file(OptionsContext *o, const char *filename)
{
    ...
    if (configure_complex_filters() < 0) {
    	LOGV("Error configuring filters.\n");
    ...
    if (!oc) {
    	LOGV("filename, AVERROR(ENOMEM) : %s", filename, AVERROR(ENOMEM));
        print_error(filename, AVERROR(ENOMEM));
        exit(1);
    }
    of->ctx = oc;
    ...
    if (o->format) {
        ...
        if (!file_oformat) {
        	LOGV("Requested output format '%s' is not a suitable output format\n", o->format);
        ...
        if (!file_oformat) {
        	LOGV("Unable to find a suitable output format for '%s'\n", filename);
...
loop_end:
                if (!ofilter) {
                	LOGV("Output with label '%s' does not exist in any defined filter graph.\n", map->linklabel);
                ...
                default:
                	LOGV("Cannot map stream #%d:%d - unsupported type.\n", map->file_index, map->stream_index);
    ...
    for (i = 0; i < o->nb_attachments; i++) {
        ...
        if ((err = avio_open2(&pb, o->attachments[i], AVIO_FLAG_READ, &int_cb, NULL)) < 0) {
        	LOGV("Could not open attachment file %s.\n", o->attachments[i]);
            av_log(NULL, AV_LOG_FATAL, "Could not open attachment file %s.\n",
                   o->attachments[i]);
            exit(1);
        }
        if ((len = avio_size(pb)) <= 0) {
        	LOGV("Could not get size of the attachment %s.\n", o->attachments[i]);
            av_log(NULL, AV_LOG_FATAL, "Could not get size of the attachment %s.\n",
                   o->attachments[i]);
            exit(1);
        }
        if (!(attachment = av_malloc(len))) {
        	LOGV("Attachment %s too large to fit into memory.\n", o->attachments[i]);
    ...
    while ((e = av_dict_get(unused_opts, "", e, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if (!(option->flags & AV_OPT_FLAG_ENCODING_PARAM)) {
        	LOGV("Codec AVOption %s (%s) specified for "
    	...
    	LOGV("Codec AVOption %s (%s) specified for output file #%d (%s) has not been used for any stream. The most "
                "likely reason is either wrong type (e.g. a video option with "
    ...
    if (oc->oformat->flags & AVFMT_NEEDNUMBER) {
        if (!av_filename_number_test(oc->filename)) {
        	LOGV("oc->filename, AVERROR(EINVAL) : %s.\n", oc->filename, AVERROR(EINVAL));
    ...
    if (!(oc->oformat->flags & AVFMT_NOFILE)) {
        ...
        if ((err = avio_open2(&oc->pb, filename, AVIO_FLAG_WRITE,
                              ...
                              &of->opts)) < 0) {
        	LOGV("filename, err : %s ", filename, err);
            print_error(filename, err);
            exit(1);
        }
    }
    ...
    for (i = 0; i < o->nb_metadata_map; i++) {
        ...
        if (in_file_index >= nb_input_files) {
        	LOGV("Invalid input file index %d while processing metadata maps\n", in_file_index);
    ...
    if (o->chapters_input_file >= nb_input_files) {
        if (o->chapters_input_file == INT_MAX) {
        	...
        	LOGV("Invalid input file index %d in chapter mapping.\n", o->chapters_input_file);
    ...
    for (i = 0; i < o->nb_metadata; i++) {
        ...
        if (!val) {
        	LOGV("No '=' character in metadata string %s.\n", o->metadata[i].u.str);
        ...
        if (type == 's') {
            ...
            case 'c':
                if (index < 0 || index >= oc->nb_chapters) {
                	LOGV("Invalid chapter index %d in metadata specifier.\n", index);
                ...
                break;
            default:
            	LOGV("Invalid metadata specifier %s.\n", o->metadata[i].specifier);
                av_log(NULL, AV_LOG_FATAL, "Invalid metadata specifier %s.\n", o->metadata[i].specifier);
                exit(1);
            }
                      ...
                      int (*open_file)(OptionsContext*, const char*))
{
    ...
    LOGV("In open_files function.\n");

    for (i = 0; i < l->nb_groups; i++) {
    LOGV("In for loop of open_files function.\n");
        ...
        if (ret < 0) {
    LOGV("Error parsing options for %s file %s.\n", inout, g->arg);
            av_log(NULL, AV_LOG_ERROR, "Error parsing options for %s file "
                   "%s.\n", inout, g->arg);
            return ret;
        }

    LOGV("Opening an %s file: %s.\n", inout, g->arg);
        av_log(NULL, AV_LOG_DEBUG, "Opening an %s file: %s.\n", inout, g->arg);
        ret = open_file(&o, g->arg);
    LOGV("After Opened an %s file: %s.\n", inout, g->arg);
    LOGV("Before uninit_options.");
        uninit_options(&o);
    LOGV("After uninit_options.");
        if (ret < 0) {
    LOGV("Error opening %s file %s.\n", inout, g->arg);
            av_log(NULL, AV_LOG_ERROR, "Error opening %s file %s.\n",
                   inout, g->arg);
            return ret;
        }
    LOGV("Successfully opened the file.\n");
