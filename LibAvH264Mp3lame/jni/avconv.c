...
static void abort_codec_experimental(AVCodec *c, int encoder)
{
    ...
    AVCodec *codec;
    LOGV(NULL, AV_LOG_FATAL, "%s '%s' is experimental and might produce bad "
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            if (!ost->enc) {
                ...
                LOGE(error, "Automatic encoder selection "
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->encoding_needed) {
            ...
            if ((ret = avcodec_open2(ost->st->codec, codec, &ost->opts)) < 0) {
                ...
                LOGE("Error while opening encoder for output stream #%d:%d - maybe incorrect parameters such as bit_rate, rate, width or height",
                        ost->file_index, ost->index);
                goto dump_format;
                return ERROR_CODE;
            }
